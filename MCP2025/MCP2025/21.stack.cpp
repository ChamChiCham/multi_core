...
thread_local int range;
std::atomic<int> g_el_count{ 0 };
...
public:
	...
	int Pop()
	{
		while (true) {
			...
			// Elimination 방문 소거 시도
			auto ret{ m_earr.Visit(RET_POP) };
			if (0 <= ret) {
				return ret;
			}
		}
